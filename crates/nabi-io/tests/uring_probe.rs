...
#![allow(
    ...
    reason = "integration test binary inherits all dev-deps; only nabi-io is used here"
)]

use nabi_io::uring::{axon::UringAxon, detect::UringCapabilities};

/// Arbitrary 64-bit tag threaded through the NOP SQE and checked on the CQE
/// side to verify `user_data` preservation across the submission/completion
/// boundary.
